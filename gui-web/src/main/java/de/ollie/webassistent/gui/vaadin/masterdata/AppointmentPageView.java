...
/**
 * A view for paginated appointment lists.
 *
 * GENERATED CODE !!! DO NOT CHANGE !!!
 */
@Generated
@Route(AppointmentPageView.URL)
@RequiredArgsConstructor
public class AppointmentPageView extends Scroller implements BeforeEnterObserver, HasUrlParameter<String> {

	public static final String URL = "webassistent/masterdata/appointments";

	private static final Logger logger = LogManager.getLogger(AppointmentPageView.class);
	private static final String PARAMETER_FILTER = "AppointmentPageView.Filter";

	@Autowired(required = false)
	private MasterDataGridFieldRenderer<Appointment> masterDataGridFieldRenderer;
	...
	public void beforeEnter(BeforeEnterEvent beforeEnterEvent) {
		...
		grid
				...
				.setHeader(resourceManager.getLocalizedString("AppointmentPageView.grid.header.title.label", session.getLocalization()))
				.setSortable(true);
		grid
				.addColumn(model -> getHeaderString("USER", model, () -> model.getUser()))
				.setHeader(resourceManager.getLocalizedString("AppointmentPageView.grid.header.user.label", session.getLocalization()))
				.setSortable(true);
		grid
				.addColumn(model -> getHeaderString("TIMESTART", model, () -> model.getTimeStart()))
				.setHeader(resourceManager.getLocalizedString("AppointmentPageView.grid.header.timestart.label", session.getLocalization()))
				.setSortable(true);
		grid
				.addColumn(model -> getHeaderString("TIMEEND", model, () -> model.getTimeEnd()))
				.setHeader(resourceManager.getLocalizedString("AppointmentPageView.grid.header.timeend.label", session.getLocalization()))
				.setSortable(true);
		...
		mainLayout.add(
				new HeaderLayout(
						...
						resourceManager.getLocalizedString("AppointmentPageView.header.label", session.getLocalization()),
	...
	private Object getHeaderString(String fieldName, Appointment aTable, Supplier<?> f) {
	...
	private void enabledButtons(SelectionEvent<Grid<Appointment>, Appointment> event) {
	...
	protected void onAttach(AttachEvent attachEvent) {
		logger.info("Appointment page layout opened for user '{}'.", session.getUserName());
	...
	private boolean isMatching(Appointment model) {
	...
	private boolean isMatchingPattern(String pattern, Appointment model) {
		boolean result = false;
		result = result || getHeaderString(Appointment.TITLE, model, () -> model.getTitle()).toString().toLowerCase().contains(pattern);
		return result;
	}

	private void addRecord() {
		session.setParameter(PARAMETER_FILTER, textFieldFilter.getValue());
		getUI().ifPresent(ui -> ui.navigate(AppointmentMaintenanceView.URL));
	}
	...
	private void duplicateRecord() {
		...
		grid.getSelectedItems().stream().findFirst().ifPresent(model -> {
			...
			getUI().ifPresent(ui -> ui.navigate(AppointmentMaintenanceView.URL, parameters));
		});
	}
